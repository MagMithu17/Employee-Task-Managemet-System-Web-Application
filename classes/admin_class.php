...
/* -------------------------set_database_connection_using_PDO---------------------- */
	...
	{ 
		...
		try{
			...
			echo htmlspecialchars($message->getMessage(), ENT_QUOTES, 'UTF-8');  // Display the error message securely using htmlspecialchars
		}
	}
...
/* ---------------------- Admin Login Check ----------------------------------- */
     ...
     public function admin_login_check($data) {
        ...
        $upass = $this->test_form_input_data(sha1(md5($data['admin_password'])));
        ...
        try
       {
		  ...
		  $stmt->bindParam(':uname', $username); // Prevents SQL Injection by binding parameters
          $stmt->bindParam(':upass', $upass); // Prevents SQL Injection by binding parameters
          $stmt->execute();
          ...
          if($stmt->rowCount() > 0)
          {
			  ...
			  $message = "Invalid user name or Password $upass";  // XSS attack prevented by not including user-input in the error message
       ...
       {
           //echo $e->getMessage();                                                        ***********************************
           echo htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8');  // Display the error message securely using htmlspecialchars
    ...
    public function change_password_for_employee($data){
		...
		$final_password = sha1(md5($password));
		$temp_password = '';
		...
		if($password == $re_password){
			...
			if (!preg_match('/^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[@#$%^&*!])[A-Za-z\d@#$%^&*!]{8,}$/', $password)) {
				$message = 'Password is not strong. It should include at least one number, one capital letter, one small letter, and one special character. It should be at least eight characters long.';
				return $message;
            }
			else{
				...
				try{
					...
					$stmt = $this->db->prepare("SELECT * FROM tbl_admin WHERE user_id=:id LIMIT 1");
					  ...
					  $stmt->bindParam(':id', $user_id); // Prevents SQL Injection by binding parameters
					  $userRow=$stmt->fetch(PDO::FETCH_ASSOC);
			...
			$message = 'Sorry !! Password Can not match';         // XSS attack prevented by not including user-input in the error message
            return $message;
		}
	...
	public function add_new_user($data){
		...
		try{
			...
			if($total_email != 0 && $total_username != 0){
				...
				$add_user->bindparam(':x', $user_fullname); // Prevents SQL Injection by binding parameters
				$add_user->bindparam(':y', $user_username); // Prevents SQL Injection by binding parameters
				$add_user->bindparam(':z', $user_email);    // Prevents SQL Injection by binding parameters
				$add_user->bindparam(':a', $user_password); // Prevents SQL Injection by binding parameters
				$add_user->bindparam(':b', $temp_password); // Prevents SQL Injection by binding parameters
				$add_user->bindparam(':c', $user_role);     // Prevents SQL Injection by binding parameters
...
/* ---------update_user_data----------*/
	...
	public function update_user_data($data, $id){
		...
		try{
			...
			$update_user->bindparam(':x', $user_fullname); // Prevents SQL Injection by binding parameters
			$update_user->bindparam(':y', $user_username); // Prevents SQL Injection by binding parameters
			$update_user->bindparam(':z', $user_email);    // Prevents SQL Injection by binding parameters
			$update_user->bindparam(':id', $id);           // Prevents SQL Injection by binding parameters
...
/* ------update_user_password------------------*/
	...
	public function update_user_password($id){
		$temp_password = rand(000000001,10000000);
		$employee_password  = sha1(md5($temp_password));
		try{
			$update_user_password = $this->db->prepare("UPDATE tbl_admin SET password = :x, temp_password = :y WHERE user_id = :id");

			$update_user_password->bindparam(':x', $employee_password); // Prevents SQL Injection by binding parameters
			$update_user_password->bindparam(':y', $temp_password);     // Prevents SQL Injection by binding parameters
			$update_user_password->bindparam(':id', $id);               // Prevents SQL Injection by binding parameters
...
/* -------------admin_password_change------------*/
	...
	public function admin_password_change($data, $id){
		$admin_old_password  = $this->test_form_input_data(sha1(md5($data['admin_old_password'])));
		$admin_new_password  = $this->test_form_input_data(sha1(md5($data['admin_new_password'])));
		$admin_cnew_password  = $this->test_form_input_data(sha1(md5($data['admin_cnew_password'])));
		...
		try{
			...
			if (!preg_match('/^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[@#$%^&*!])[A-Za-z\d@#$%^&*!]{8,}$/', $admin_raw_password)) {
                $all_error .= '<br>'."Password is not strong. It should include at least one number, one capital letter, one small letter, and one special character. It should be at least eight characters long.";
            }

			if(empty($all_error)){
				$update_admin_password = $this->db->prepare("UPDATE tbl_admin SET password = :x WHERE user_id = :id ");

				$update_admin_password->bindparam(':x', $admin_new_password); // Prevents SQL Injection by binding parameters
				$update_admin_password->bindparam(':id', $id);                // Prevents SQL Injection by binding parameters
	...
	public function add_new_task($data){
		...
		try{
			...
			$add_task->bindparam(':x', $task_title);        // Prevents SQL Injection by binding parameters
			$add_task->bindparam(':y', $task_description);	// Prevents SQL Injection by binding parameters
			$add_task->bindparam(':z', $t_start_time);		// Prevents SQL Injection by binding parameters
			$add_task->bindparam(':a', $t_end_time);		// Prevents SQL Injection by binding parameters
			$add_task->bindparam(':b', $assign_to);			// Prevents SQL Injection by binding parameters
		...
		public function update_task_info($data, $task_id, $user_role){
			...
			try{
				...
				$update_task->bindparam(':x', $task_title);  	    // Prevents SQL Injection by binding parameters
				$update_task->bindparam(':y', $task_description);   // Prevents SQL Injection by binding parameters
				$update_task->bindparam(':z', $t_start_time);		// Prevents SQL Injection by binding parameters
				$update_task->bindparam(':a', $t_end_time);			// Prevents SQL Injection by binding parameters
				$update_task->bindparam(':b', $assign_to);			// Prevents SQL Injection by binding parameters
				$update_task->bindparam(':c', $status);				// Prevents SQL Injection by binding parameters
				$update_task->bindparam(':id', $task_id);			// Prevents SQL Injection by binding parameters
	...
	public function add_punch_out($data){
		...
		try{
			...
			$update_user->bindparam(':x', $punch_out_time);  // Prevents SQL Injection by binding parameters
			$update_user->bindparam(':y', $total_duration);  // Prevents SQL Injection by binding parameters
			$update_user->bindparam(':id', $attendance_id);  // Prevents SQL Injection by binding parameters
	...
	public function delete_data_by_this_method($sql,$action_id,$sent_po){
		try{
			...
			$delete_data->bindparam(':id', $action_id);  // Prevents SQL Injection by binding parameters
